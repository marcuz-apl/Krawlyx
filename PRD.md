# zenCrawl — Product Requirements Document
...
zenCrawl is a self-hosted web scraping workbench. A **runner** pastes one or more
...
- G7 — **Zero-cost**: every bundled component is free/open source; no paid API is
  required for any core feature. zenCrawl itself is released under MIT.
...
- The database file lives at **`./data/zencrawl.db`** (overridable via settings/env);
  zenCrawl creates `./data/` on first run. The folder is tracked in git; only the
...
There is no budget for paid services; zenCrawl exists for the public good and is
released under MIT.
...
- **FR-EXP-07** — Naming: `zenCrawl_{jobslug}_{YYYYMMDD-HHMMSS}_part{n:03d}.{csv|xlsx}`; a `_manifest.json` beside the parts lists columns, row counts, sizes, and job id.
...
```text
┌───────────────────────────────────────────────────────────┐
│ zenCrawl   [New job] [History]                 runner ▾   │
├───────────────────────────────────────────────────────────┤
...
- **NFR-05 Compliance** — robots.txt respected by default; identifiable User-Agent `zenCrawl/0.1 (+{admin contact})`.
...
| Risk | Mitigation |
| --- | --- |
| Playwright browser install friction on Windows | Document `crawl4ai-setup`; ship doctor command `python -m zencrawl.doctor` |
