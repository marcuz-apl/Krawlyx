...
def _is_superadmin(user: User) -> bool:
    return user.role == "superadmin" or (user.username == "admin" and user.role in ("admin", "superadmin"))
...
def patch_user_route(
    ...
    if body.role == "superadmin" and not _is_superadmin(current_admin):
        raise HTTPException(status_code=403, detail="Only SuperAdmin can promote accounts to SuperAdmin")

    if body.password is not None:
        row.password_hash = hash_password(body.password)
    if body.role is not None and body.role != row.role:
        if row.role in ("admin", "superadmin") and body.role == "runner" and _count_privileged_admins(db) <= 1:
