"""Patchtroy adapter unit tests — exercise the contract without the browser.
...
def test_patchtroy_registers_under_canonical_type() -> None:
    """The adapter module is the source of truth for engine registration."""
    assert "patchtroy" in registry.available_types()
    caps = registry.capabilities_for("patchtroy")
    assert caps is not None
    assert caps.deep_crawl is True
    assert caps.supports_render is True


def test_patchtroy_factory_returns_protocol_compatible_instance() -> None:
    engine = registry.build("patchtroy", {"headless": True, "browser_timeout_s": 5})
    assert isinstance(engine, CrawlEngine)
    assert engine.type == "patchtroy"
    assert engine.capabilities.supports_render


def test_patchtroy_rejects_bad_config() -> None:
    from app.engines.patchtroy_engine import PatchtroyEngine

    with pytest.raises(ValidationError):
        PatchtroyEngine(config={"browser_timeout_s": 99999})


def test_patchtroy_skips_blocked_targets(monkeypatch) -> None:
...
def test_patchtroy_markdown_extraction_with_trafilatura() -> None:
    ...
    <html>
      ...
      <body>
        ...
        <article>
          <h1>Patchtroy Architecture</h1>
          <p>Patchtroy pairs Patchright with Trafilatura for stealth scraping.</p>
        </article>
    ...
    assert "Patchtroy Architecture" in md
    assert "Patchright with Trafilatura" in md
