...
export function DatabaseBrowser() {
  ...
  const loadTableRows = useCallback(async () => {
    ...
    try {
      ...
      console.error(`Failed to load rows for ${selectedTable}`, err);
    } finally {
      setLoadingTable(false);
    }
  }, [selectedTable, page, pageSize, sortCol, sortDir, search]);
  ...
  }, [activeTab, selectedTable, page, pageSize, sortCol, sortDir, search, loadTableRows]);

  // Run Raw SQL Query
  const handleExecuteSql = async () => {
    if (!sqlQuery.trim() || runningQuery) return;
  ...
  const handleRunMaintenance = async (action: 'vacuum' | 'checkpoint' | 'optimize' | 'integrity_check') => {
    ...
    try {
      ...
      setActionFeedback(`Error: ${err.response?.data?.detail || err.message}`);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 text-white p-6 shadow-xl relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition-all flex items-center gap-1.5 shadow-sm"
            ...
            <button
              ...
              className="p-1.5 rounded-lg text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-700 transition-all"
        ...
        {/* Stats Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mt-5 pt-4 border-t border-slate-800/80 text-xs">
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">Database Size</span>
            <span className="font-bold text-sm text-indigo-300">{stats?.file_size_formatted || '0.00 MB'}</span>
          </div>
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">WAL Journal</span>
            <span className="font-bold text-sm text-blue-300">{stats?.wal_size_formatted || '0.00 MB'}</span>
          </div>
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">Total Tables</span>
            <span className="font-bold text-sm text-slate-200">{tables.length} tables</span>
          </div>
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">Page Count</span>
            <span className="font-bold text-sm text-slate-200">{stats?.page_count?.toLocaleString() || 0}</span>
          </div>
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">Page Size</span>
            <span className="font-bold text-sm text-slate-200">{stats?.page_size ? `${stats.page_size / 1024} KB` : '4 KB'}</span>
          </div>
          <div className="bg-slate-900/60 rounded-xl p-2.5 border border-slate-800">
            <span className="text-slate-400 text-[11px] block">Integrity</span>
            <span className={`font-bold text-sm ${stats?.integrity_ok ? 'text-emerald-400' : 'text-amber-400'}`}>
        ...
        {actionFeedback && (
          <div className="mt-3 p-3 rounded-xl bg-indigo-950/80 border border-indigo-500/40 text-xs text-indigo-200 flex items-center gap-2 animate-in fade-in">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        {activeTab === 'tables' && tableData && tableData.rows.length > 0 && (
          <button
            ...
            className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm transition-all flex items-center gap-1.5"
          >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-all flex items-center gap-1"
                    >
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="w-full text-left text-xs border-collapse font-mono">
                <thead className="bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-slate-300 font-bold sticky top-0 z-10 border-b border-slate-200 dark:border-slate-800 shadow-sm">
                  <tr>
                    {tableData?.columns.map(col => (
                      <th
                        ...
                        className="px-3 py-2.5 cursor-pointer hover:bg-slate-200/70 dark:hover:bg-slate-800 transition-all select-none whitespace-nowrap"
                      >
                        <div className="flex items-center gap-1.5">
                          <span>{col.name}</span>
                          {col.pk && (
                            <span className="px-1 rounded text-[9px] bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 font-bold">
                              PK
                            </span>
                          )}
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {loadingTable ? (
                    ...
                    <tr>
                      <td colSpan={tableData?.columns.length || 1} className="py-12 text-center text-slate-400">
                        No rows found in <span className="font-bold">{selectedTable}</span> {search && `matching "${search}"`}
                      </td>
                    </tr>
                  ) : (
                      ...
                      >
                        {tableData.columns.map(col => {
                          ...
                          return (
                            <td
                              key={col.name}
                              className="px-3 py-2 whitespace-nowrap max-w-xs truncate text-slate-800 dark:text-slate-200"
                            >
                              {isNull ? (
                                ...
                                <span className={val ? 'text-emerald-500 font-bold' : 'text-slate-400'}>
                                  {String(val)}
                                </span>
                              ) : (
                                String(val)
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 text-xs">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="p-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 disabled:opacity-40 hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
      ...
      {activeTab === 'sql' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs">
            <span className="text-slate-500 font-semibold px-2">Quick Presets:</span>
            {[
          ...
          {/* SQL Editor Area */}
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-950 text-slate-100 overflow-hidden shadow-lg">
            <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-slate-800 text-xs text-slate-400">
              ...
              <span className="text-[11px] text-slate-500">
                Press <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-slate-300 font-mono">Ctrl+Enter</kbd> to run
              </span>
            </div>
            ...
            <textarea
              ...
              className="w-full p-4 bg-transparent text-emerald-400 font-mono text-sm focus:outline-none resize-y"
              spellCheck={false}
            />
            ...
            <div className="flex items-center justify-between p-3 bg-slate-900/80 border-t border-slate-800">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-1.5 rounded-lg text-xs font-bold bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white transition-all flex items-center gap-2 shadow-md shadow-indigo-600/30"
                >
                  {runningQuery ? (
                    ...
                    <table className="w-full text-left text-xs border-collapse font-mono">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                        {queryResult.rows.map((row, rIdx) => (
                          <tr key={rIdx} className="hover:bg-indigo-50/40 dark:hover:bg-indigo-950/20">
                            {queryResult.columns!.map(col => {
                              const val = row[col];
                              const isNull = val === null || val === undefined;
                              return (
      ...
      {cellModalValue && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <pre className="p-4 rounded-xl bg-slate-950 text-slate-100 text-xs font-mono max-h-96 overflow-y-auto whitespace-pre-wrap break-words border border-slate-800">
      ...
      {showDdlModal && selectedTableMeta && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <pre className="p-4 rounded-xl bg-slate-950 text-emerald-400 text-xs font-mono max-h-80 overflow-y-auto whitespace-pre-wrap border border-slate-800">
