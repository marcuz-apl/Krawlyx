...
import { Clock, Sparkles, HelpCircle, Calendar, X, Globe } from 'lucide-react';
...
export function ScheduleFormModal({ schedule, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
      <form
        onSubmit={onSubmit}
        className="w-full max-w-5xl space-y-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 sm:p-7 shadow-2xl animate-in zoom-in-95 my-4 max-h-[94vh] flex flex-col justify-between overflow-y-auto"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3.5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/70 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/40">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Automated background crawl execution powered by in-process APScheduler
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex justify-end gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
          ...
          <button
            type="submit"
            disabled={pending}
            className="rounded-xl bg-indigo-600 hover:bg-indigo-500 px-5 py-2 text-xs font-semibold text-white shadow-sm disabled:opacity-50 transition-colors"
          >
